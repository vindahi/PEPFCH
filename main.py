...
def FedProto_taskheter(opt, X, Y, L, user_groups, img_model, txt_model, S, local_F_buffer_dict,
    ...
    if opt.valid:
        max_mapi2t_avg = max(max_mapi2t)
        max_mapt2i_avg = max(max_mapt2i)
...
def update_hypernetwork(hypernet, client_model_params_list, client_id, diff, opt):
    ...
    # Check if the client has any new data
    if len(diff) > 0:
        # Calculate gradients
        hn_grads = torch.autograd.grad(
            ...
            allow_unused=True,
            create_graph=True,  # Required for accumulating gradients
        )
        ...
        # Extend hypernetwork parameters if necessary
        num_new_fc_layers = len(fc_grads) - len(hypernet.fc_layer_parameters())
        num_new_mlp_layers = len(mlp_grads) - len(hypernet.mlp_parameters())
        num_new_emd_layers = len(emd_grads) - len(hypernet.emd_parameters())

        if num_new_fc_layers > 0:
            hypernet.extend_fc_layers(num_new_fc_layers)
        if num_new_mlp_layers > 0:
            hypernet.extend_mlp_layers(num_new_mlp_layers)
        if num_new_emd_layers > 0:
            hypernet.extend_emd_layers(num_new_emd_layers)

        # Accumulate gradients and update hypernetwork parameters
        ...
        for param in hypernet.mlp_parameters() + hypernet.fc_layer_parameters() + hypernet.emd_parameters():
            if param.grad is not None:
                param.data += opt.hn_lr * param.grad
                param.grad = None  # Reset gradients after applying
